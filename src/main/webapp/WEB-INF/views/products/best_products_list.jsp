...
<body>
			...
			<div class="product-card">
				...
				<div class="product-title">
					<a
						...
						class="product-name">${vo.product_name}</a>
				</div>
				<div class="product-price">
					<span><fmt:formatNumber value="${vo.final_price}"
							type="number" />원
						<div>
							<img alt=""
	...
	<script>
		...
		// Store product_ids from the model in a JavaScript array
		const initialProductIds = [<c:forEach var='id' items='${product_ids}' varStatus='status'>${id}${!status.last ? ',' : ''}</c:forEach>];
	</script>
	<script
		src="${pageContext.request.contextPath}/resources/products/script/product_to_cart.js"></script>
	<script
		src="${pageContext.request.contextPath}/resources/products/script/product_list_common.js"></script>
	<script>
		// 초기 로드 시 장바구니/찜 기능 초기화
		if (typeof initializeProductListActions === 'function') {
			initializeProductListActions(contextPath);
		}
		...
		document.addEventListener('DOMContentLoaded', () => {
			...
			if (!productListContainer || sortLinks.length === 0 || typeof initialProductIds === 'undefined') {
				return;
			}
			...
			sortLinks.forEach(link => {
				link.addEventListener('click', (e) => {
					...
					const productIdsParam = initialProductIds.join(',');

					if (!sortValue || !productIdsParam) {
						console.error('Sort value or product IDs are missing.');
						return;
					}

					const url = `${contextPath}/best/sortProducts?sort=${sortValue}&product_ids=${productIdsParam}`;
					...
					fetch(url)
						...
						.catch(error => {
							console.error('Error fetching sorted best products:', error);
						});
				});
			});
		});
